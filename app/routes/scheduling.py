...
async def public_booking_page(request: Request, slug: str) -> HTMLResponse:
    ...
    if signed_in:
        page = page.replace(
            "<body>",
            '<body><div class="page-account" id="account">'
...
PUBLIC_INFO_HTML = r"""<section class="booking-info"><div><h3>Pricing</h3><p class="muted">Single lesson — €30<br>8 lessons — €224 (€28/hour)<br>12 lessons — €324 (€27/hour)<br>20 lessons — €500 (€25/hour)</p></div><div><h3>Booking conditions</h3><p class="muted">Book one session as a guest, or sign in with Google to book up to 10 lessons at once. Packages are valid for five weeks from the first booked lesson. Cancellations and rescheduling at least 12 hours ahead restore the lesson credit. Later changes remain available but consume the credit.</p></div></section>"""
...
.page-account{position:fixed;top:20px;right:24px;z-index:10}.signed-in{display:flex;align-items:center;gap:9px;padding:6px 7px 6px 11px;border:1px solid #dedff1;border-radius:13px;background:rgba(255,255,255,.94);box-shadow:0 8px 24px rgba(75,73,145,.1);backdrop-filter:blur(10px)}.signed-in span{max-width:220px;color:#716d87;font-size:11px;overflow:hidden;text-overflow:ellipsis}.signed-in strong{color:#494566}.signed-in form{display:flex}.signed-in button{min-height:32px;padding:6px 9px;border:1px solid #dedff1;border-radius:9px;background:#fff;color:#5549b8;font-size:11px;font-weight:750;cursor:pointer}
...
.signin-actions{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.signin-actions button.secondary{min-height:42px;padding:10px 16px;border:1px solid var(--line);border-radius:11px;background:#fff;color:var(--accent-dark);font-weight:750;cursor:pointer}.guest-benefits{margin:10px 0 0!important;font-size:12px!important}.guest-details{padding:14px 16px;border:1px solid var(--line);border-radius:14px;background:linear-gradient(135deg,#f8faff,#fbf7ff)}.calendar{grid-template-columns:repeat(7,minmax(36px,52px));justify-content:space-between;gap:8px 6px}@media(max-width:650px){body{padding-top:64px}.page-account{top:10px;right:10px}.signed-in span{max-width:180px}}@media(max-width:420px){.calendar{grid-template-columns:repeat(7,minmax(30px,1fr));justify-content:normal;gap:4px}}
...
async function load(){profile=await api(`/api/public/scheduling/${slug}`);$('#title').textContent=`Book a lesson with ${profile.display_name}`;if(!signedIn){$('#app').classList.add('hidden');$('#my-lessons').classList.add('hidden');$('.shell').classList.add('signin-mode');$('#login').classList.remove('hidden');return}await initializeBooking()}
