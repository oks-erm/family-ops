...
.availability-card .list{gap:10px}.days{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto;gap:8px;padding:10px;border:1px solid var(--line);border-radius:16px;background:var(--purple-pale)}.days select{grid-column:1/3}.days input{min-width:0}.days .danger{grid-column:3;grid-row:1/3;align-self:stretch;min-height:0;padding:8px 11px}.availability-card>.row{margin-top:16px}.status{position:fixed;left:50%;bottom:22px;z-index:10;min-height:0;transform:translateX(-50%);padding:10px 16px;border-radius:999px;background:#302443;color:#fff;box-shadow:0 10px 30px rgba(48,36,67,.2)}.status:empty{display:none}.hidden{position:absolute;left:-9999px}
...
<section class="card availability-card"><h2>Weekly availability</h2><p class="muted">Add one or more windows per day. Times use your timezone.</p><div class="list" id="availability"></div><div class="row"><button id="add-window" class="secondary" type="button">Add window</button><button id="save-availability" type="button">Save availability</button></div></section>
