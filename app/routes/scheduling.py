...
<title>Free lesson scheduling for tutors</title><style>
:root{font-family:"Google Sans",Inter,ui-sans-serif,system-ui,-apple-system,sans-serif;color:#29213a;background:#f8f6fc;--purple:#7357c7;--purple-dark:#6042b3;--purple-soft:#eee9fb;--line:#e6e0ef;--muted:#756d82}*{box-sizing:border-box}body{margin:0;min-height:100vh;padding:64px 24px;background:radial-gradient(circle at 8% 0%,#eee8ff 0,transparent 30%),radial-gradient(circle at 96% 10%,#f6eafb 0,transparent 28%),#f8f6fc}main{width:min(1120px,100%);margin:auto}.hero{display:grid;grid-template-columns:minmax(0,1.4fr) minmax(280px,.6fr);gap:22px;align-items:start}.card{background:rgba(255,255,255,.95);border:1px solid rgba(221,212,235,.9);border-radius:24px;padding:34px;box-shadow:0 16px 42px rgba(67,47,98,.09)}h1{max-width:760px;margin:0 0 18px;font-size:clamp(38px,6vw,64px);line-height:1.02;letter-spacing:-.045em}h2{margin:0 0 14px;font-size:23px}p{color:var(--muted);font-size:16px;line-height:1.68}.lead{font-size:19px;color:#4f435f}.actions{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-top:26px}.primary,.secondary{min-height:44px;display:inline-flex;align-items:center;justify-content:center;padding:11px 18px;border-radius:999px;font-weight:800;text-decoration:none}.primary{background:linear-gradient(135deg,#6372d8,#8559cf);color:#fff;box-shadow:0 8px 20px rgba(96,66,179,.22)}.secondary{background:var(--purple-soft);border:1px solid #e2d9f7;color:#584396}.coffee{text-align:center}.coffee .qr{display:block;width:min(210px,78%);height:auto;margin:0 auto 18px;border-radius:18px}.coffee p{margin-bottom:18px}.steps{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin-top:22px}.step{padding:20px;border:1px solid var(--line);border-radius:18px;background:rgba(255,255,255,.9)}.step strong,.step span{display:block}.step strong{color:#3f3551}.step span{margin-top:7px;color:var(--muted);font-size:13px;line-height:1.5}@media(max-width:850px){.hero{grid-template-columns:1fr}.steps{grid-template-columns:1fr 1fr}}@media(max-width:560px){body{padding:32px 14px}.card{padding:24px}.steps{grid-template-columns:1fr}}
</style></head><body><main><section class="hero"><section class="card"><h1>Less scheduling admin. More actual teaching.</h1><p class="lead">I built this free scheduling service while tutoring because I know the pain: too much admin, conversations scattered across different apps, and several free-tier tools held together with digital duct tape.</p><p>It helps tutors manage bookings, keep track of payments and income, and let students schedule several lessons at once. Rescheduling and cancellations are simpler too—and your cancellation policy is shown clearly during booking, so nobody has to negotiate the rules after something changes.</p><p>There’s no subscription, and the service never processes your payments. Lessons stay in your calendar and your students’ calendars, while you remain in control of payments.</p><p>There’s no lock-in either. If the service disappears in a dramatic puff of experimental software, your calendar events remain where they are—you can simply return to arranging lessons the old-fashioned way.</p><p>This is an early version that I’m sharing for free while I test and improve it. If it saves you some admin, wonderful. If you find something confusing or broken, that is useful too—please tell me.</p><div class="actions">__LANDING_ACTION__</div></section><aside class="card coffee"><h2>Help shape what comes next</h2><img class="qr" src="/api/scheduling/assets/coffee-qr.png" alt="QR code for Buy Me a Coffee"><p>Have a feature idea? Send it with a coffee.</p><a class="secondary" href="https://www.buymeacoffee.com/okserm" target="_blank" rel="noopener noreferrer">Open Buy Me a Coffee</a></aside></section><section class="steps" aria-label="How to get started"><div class="step"><strong>Connect your calendars</strong><span>Add every calendar that should block unavailable times.</span></div><div class="step"><strong>Choose where lessons are created</strong><span>Select a writable Google calendar for invitations and Meet links.</span></div><div class="step"><strong>Add lesson types and availability</strong><span>Set lesson lengths, teaching hours, notice, and non-lesson travel buffers.</span></div><div class="step"><strong>Share your booking page</strong><span>Copy your public link when the schedule looks right.</span></div></section></main></body></html>"""
...
.dashboard-footer{position:sticky;bottom:16px;z-index:8;display:flex;align-items:center;justify-content:center;gap:12px;width:max-content;max-width:calc(100% - 24px);margin:30px auto 0;padding:9px 11px;border:1px solid rgba(213,203,231,.9);border-radius:999px;background:rgba(255,255,255,.88);box-shadow:0 14px 38px rgba(65,48,96,.16);backdrop-filter:blur(14px)}.dashboard-footer .bug-link{display:inline-flex;align-items:center;min-height:38px;padding:8px 14px;border:1px solid #ded5ed;border-radius:999px;background:#f7f3ff;color:var(--purple-dark);font-size:13px;font-weight:800;text-decoration:none}.dashboard-footer .bug-link:hover{background:#eee7fb}
...
<footer class="dashboard-footer"><script type="text/javascript" src="https://cdnjs.buymeacoffee.com/1.0.0/button.prod.min.js" data-name="bmc-button" data-slug="okserm" data-color="#FFDD00" data-emoji="☕" data-font="Cookie" data-text="Buy me a coffee" data-outline-color="#000000" data-font-color="#000000" data-coffee-color="#ffffff"></script><a class="bug-link" href="/schedule/feedback">Report a bug</a></footer><p class="status" id="status"></p></main><dialog class="confirm-dialog" id="confirm-dialog" aria-labelledby="confirm-title" aria-describedby="confirm-message"><div class="confirm-body"><div class="confirm-icon" aria-hidden="true">!</div><h2 id="confirm-title"></h2><p id="confirm-message"></p><div class="confirm-actions"><button class="secondary" id="confirm-back" type="button">Go back</button><button class="danger" id="confirm-action" type="button"></button></div></div></dialog>
<script>
...
ACCOUNT_CONTROL_CSS = TUTOR_ACCOUNT_CSS
