...
class BugReportRequest(BaseModel):
    section: Literal["welcome", "setup", "settings", "lessons", "other"]
...
TUTOR_ACCOUNT_CSS = (
    ".page-account{position:sticky;top:0;z-index:20;display:flex;justify-content:flex-end;"
    "padding:10px 24px;background:rgba(248,246,252,0.95);backdrop-filter:blur(8px);"
    "border-bottom:1px solid rgba(221,212,235,0.7)}"
    ...
    "@media(max-width:650px){.page-account{padding:8px 14px}"
    ".signed-in span{max-width:180px}}"
)


def _tutor_account_control(*, email: str) -> str:
    if email:
        safe_email = html.escape(email)
        content = (
            '<div class="signed-in"><span>Signed in as '
            f"<strong>{safe_email}</strong></span>"
            '<form method="post" action="/auth/logout?next=scheduling">'
            '<button type="submit">Sign out</button></form></div>'
        )
    else:
        content = (
            '<a class="account-sign-in" href="/auth/google/start?next=scheduling">'
            "Sign in</a>"
        )
    return f'<div class="page-account" id="tutor-account">{content}</div>'


def _render_tutor_page(page: str, *, email: str) -> str:
    return page.replace("</style>", TUTOR_ACCOUNT_CSS + "</style>", 1).replace(
        "<body>", f"<body>{_tutor_account_control(email=email)}", 1
    )


TUTOR_LANDING_HTML = r"""<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Scheduling for tutors</title><style>
:root{font-family:"Google Sans",Inter,ui-sans-serif,system-ui,-apple-system,sans-serif;color:#28213a;--purple:#7357c7;--purple-dark:#6042b3;--purple-soft:#eee9fb;--line:#e6e0ef;--muted:#756d82}
*{box-sizing:border-box}
body{margin:0;min-height:100vh;background:radial-gradient(ellipse at 16% 0%,rgba(221,212,255,.48) 0%,transparent 44%),radial-gradient(ellipse at 86% 5%,rgba(240,224,252,.42) 0%,transparent 40%),#f8f6fc}
main{width:min(920px,100%);margin:auto;padding:0 24px 72px}
.hero{padding:60px 0 44px;max-width:600px}
.tag{display:inline-flex;align-items:center;padding:4px 12px;border-radius:999px;background:var(--purple-soft);color:var(--purple-dark);font-size:11.5px;font-weight:700;letter-spacing:.04em;margin-bottom:20px}
h1{font-size:clamp(30px,4.5vw,48px);line-height:1.1;letter-spacing:-.04em;margin:0 0 16px;font-weight:800}
.hero-desc{font-size:16.5px;color:var(--muted);line-height:1.72;margin:0 0 10px;max-width:500px}
.hero-note{font-size:13.5px;color:var(--muted);opacity:.75;margin:0 0 28px;max-width:480px}
.actions{display:flex;gap:10px;flex-wrap:wrap}
a.primary{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:11px 22px;border-radius:999px;font-weight:750;font-size:15px;text-decoration:none;background:linear-gradient(135deg,#6372d8,#8559cf);color:#fff;box-shadow:0 6px 18px rgba(96,66,179,.22)}
a.secondary{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:11px 22px;border-radius:999px;font-weight:750;font-size:15px;text-decoration:none;background:var(--purple-soft);border:1px solid #e2d9f7;color:#584396}
.sep{height:1px;background:var(--line);margin:4px 0}
.features{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;padding:36px 0 32px}
.feature{padding:22px;background:rgba(255,255,255,.96);border:1px solid var(--line);border-radius:18px}
.f-icon{font-size:18px;margin-bottom:10px}
.feature h3{font-size:14.5px;font-weight:700;margin:0 0 8px;color:#3a3054}
.feature p{font-size:13.5px;color:var(--muted);line-height:1.6;margin:0}
.getting-started{padding:0 0 36px}
.section-label{font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--purple-dark);margin:0 0 16px}
.steps{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}
.step{padding:18px 20px;background:rgba(255,255,255,.9);border:1px solid var(--line);border-radius:16px}
.step-n{display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:8px;background:var(--purple-soft);color:var(--purple-dark);font-size:12px;font-weight:800;margin-bottom:10px}
.step h3{font-size:13.5px;font-weight:700;margin:0 0 6px;color:#3a3054}
.step p{font-size:12.5px;color:var(--muted);line-height:1.55;margin:0}
footer.landing{display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;padding:20px 0;border-top:1px solid var(--line);font-size:13px;color:var(--muted)}
footer.landing a{color:var(--purple-dark);font-weight:700;text-decoration:none}
footer.landing a:hover{text-decoration:underline}
@media(max-width:700px){.features{grid-template-columns:1fr 1fr}.steps{grid-template-columns:1fr 1fr}}
@media(max-width:460px){main{padding:0 16px 52px}.hero{padding:40px 0 28px}.features,.steps{grid-template-columns:1fr}}
</style></head><body>
<main>
<section class="hero">
<span class="tag">Free for tutors · No subscription</span>
<h1>Less scheduling admin. More actual teaching.</h1>
<p class="hero-desc">I built this while tutoring because the admin kept getting in the way. Students book directly from your page, everything lands in your calendar, and cancellations handle themselves.</p>
<p class="hero-note">No lock-in — if this service ever disappears, your calendar events stay exactly where they are.</p>
<div class="actions">__LANDING_ACTION__</div>
</section>
<div class="sep"></div>
<section class="features" aria-label="What you get">
<div class="feature"><div class="f-icon">📅</div><h3>Direct booking</h3><p>Students book from your real availability — up to ten lessons at once, with your calendar already factored in and Google Meet links created automatically.</p></div>
<div class="feature"><div class="f-icon">💳</div><h3>Payments and credits</h3><p>Register payments, assign lesson credits, and see monthly earned income at a glance. The service never touches your payments — that stays between you and your students.</p></div>
<div class="feature"><div class="f-icon">🔁</div><h3>Cancellations, clearly</h3><p>Your policy is shown during booking so there is nothing to negotiate later. Students reschedule or cancel themselves; credits are restored when the policy allows.</p></div>
</section>
<section class="getting-started" aria-label="How to get started">
<p class="section-label">Getting started</p>
<div class="steps">
<div class="step"><span class="step-n">1</span><h3>Sign in with Google</h3><p>One sign-in to verify your identity. It stays private — students never see your email.</p></div>
<div class="step"><span class="step-n">2</span><h3>Connect calendars</h3><p>Add Google and iCloud calendars. Anything already scheduled automatically blocks availability.</p></div>
<div class="step"><span class="step-n">3</span><h3>Set your hours</h3><p>Choose the days and times you teach. Buffers apply around non-lesson events; lessons can run back-to-back.</p></div>
<div class="step"><span class="step-n">4</span><h3>Share your link</h3><p>Send students your booking link. New lessons go straight to your calendar with invitations sent.</p></div>
</div>
</section>
<footer class="landing">
<span>Early version, shared for free. If it saves you some admin, wonderful.</span>
<a href="https://www.buymeacoffee.com/okserm" target="_blank" rel="noopener noreferrer">Buy me a coffee ☕</a>
</footer>
</main>
</body></html>"""
...
async def tutor_registration_page(request: Request):
    ...
    pending_email = str(request.session.get("pending_tutor_email") or "").strip().casefold()
    return HTMLResponse(_render_tutor_page(REGISTRATION_HTML, email=pending_email))
...
async def scheduling_admin_page(request: Request) -> HTMLResponse:
    email = _require_superadmin(request)
    return HTMLResponse(_render_tutor_page(ADMIN_HTML, email=email))
...
async def scheduling_management_page(request: Request):
    ...
    return HTMLResponse(_render_tutor_page(page, email=tutor_email))


@router.get("/schedule/feedback", response_class=HTMLResponse)
async def scheduling_feedback_page(request: Request):
    tutor_email = str(request.session.get("google_email") or "").strip().casefold()
    if not tutor_email:
        return RedirectResponse("/auth/google/start?next=scheduling", status_code=303)
    settings = get_settings()
    feedback_service = SchedulingFeedbackService(settings)
    page = BUG_REPORT_HTML.replace(
...
async def scheduling_coffee_qr() -> FileResponse:
    return FileResponse(
        COFFEE_QR_PATH,
...
BUG_REPORT_HTML = r"""<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Report a scheduling bug</title><style>
:root{font-family:"Google Sans",Inter,ui-sans-serif,system-ui,-apple-system,sans-serif;color:#29213a;background:#f8f6fc;--purple:#7357c7;--purple-dark:#6042b3;--line:#e6e0ef;--muted:#756d82}*{box-sizing:border-box}body{margin:0;min-height:100vh;padding:72px 24px;background:radial-gradient(circle at 8% 0%,#eee8ff 0,transparent 30%),radial-gradient(circle at 96% 10%,#f6eafb 0,transparent 28%),#f8f6fc}main{width:min(720px,100%);margin:auto}.back{display:inline-flex;margin-bottom:18px;color:var(--purple-dark);font-weight:750;text-decoration:none}.card{padding:32px;background:rgba(255,255,255,.96);border:1px solid var(--line);border-radius:22px;box-shadow:0 16px 42px rgba(67,47,98,.09)}h1{margin:0 0 10px;font-size:34px}p{color:var(--muted);line-height:1.55}form{display:grid;gap:16px;margin-top:24px}label{display:grid;gap:7px;font-size:13px;font-weight:750}select,textarea,input,button{font:inherit}select,textarea,input{width:100%;padding:12px;border:1px solid #d8d0e5;border-radius:11px;background:#fff;color:#29213a}textarea{min-height:170px;resize:vertical}button{justify-self:start;min-height:42px;padding:10px 17px;border:0;border-radius:999px;background:linear-gradient(135deg,#6372d8,#8559cf);color:#fff;font-weight:800;cursor:pointer}.turnstile-row{display:flex;align-items:center;gap:14px;flex-wrap:wrap}.note{font-size:12px}.hp{position:absolute;left:-9999px}.unavailable{padding:12px;border-radius:12px;background:#fff7e5;color:#725813;font-size:13px}.status{min-height:22px;margin:16px 0 0;color:var(--purple-dark);font-weight:750}@media(max-width:560px){body{padding:70px 14px 28px}.card{padding:24px}.turnstile-row{align-items:flex-start;flex-direction:column}}
</style></head><body><main><a class="back" href="/schedule/manage">← Back to dashboard</a><section class="card"><h1>Report a bug</h1><p>Tell me what happened. Your verified sign-in email is included privately so I can follow up; it is never displayed here.</p><form id="bug-report"><label>Where did it happen?<select name="section"><option value="lessons">Lessons</option><option value="settings">Settings</option><option value="other">Somewhere else</option></select></label><label class="hp">Website<input name="website" tabindex="-1" autocomplete="off"></label><label>What happened?<textarea name="message" minlength="20" maxlength="4000" required placeholder="What were you trying to do, and what happened instead?"></textarea></label><div class="turnstile-row"><div class="cf-turnstile" data-sitekey="__TURNSTILE_SITE_KEY__" data-action="bug-report" data-theme="light"></div><button id="send-bug-report" type="submit">Send bug report</button></div><span class="note">Please do not include passwords, calendar links, or private student information.</span><p class="unavailable" id="feedback-unavailable" hidden>Bug reporting is temporarily unavailable.</p></form><p class="status" id="status" role="status" aria-live="polite"></p></section></main><script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer></script><script>
const form=document.querySelector('#bug-report'),statusNode=document.querySelector('#status'),feedbackConfigured=__FEEDBACK_CONFIGURED__;function status(message){statusNode.textContent=message}if(!feedbackConfigured){document.querySelector('#send-bug-report').disabled=true;document.querySelector('#feedback-unavailable').hidden=false;document.querySelector('.cf-turnstile').hidden=true}form.onsubmit=async event=>{event.preventDefault();if(!feedbackConfigured)return;const data=new FormData(form),button=document.querySelector('#send-bug-report'),token=String(data.get('cf-turnstile-response')||'');if(!token){status('Please complete the security check.');return}button.disabled=true;status('');try{const response=await fetch('/api/scheduling/bug-reports',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({section:data.get('section'),message:data.get('message'),website:data.get('website'),turnstile_token:token})});if(!response.ok){let message='Could not send the bug report.';try{message=(await response.json()).detail||message}catch{}throw Error(message)}form.reset();if(window.turnstile)turnstile.reset();status('Bug report sent. Thank you!')}catch(error){if(window.turnstile)turnstile.reset();status(error.message)}finally{button.disabled=false}};
</script></body></html>"""
...
main{max-width:1280px;margin:auto;padding:32px 28px 80px}header{display:flex;align-items:center;gap:20px;margin-bottom:24px}.header-actions{display:flex;align-items:center;gap:16px;margin-left:auto}.admin-link{color:var(--purple-dark);font-size:13px;font-weight:750;text-decoration:none;white-space:nowrap}.admin-link:hover{text-decoration:underline}
...
.confirm-dialog{width:min(440px,calc(100% - 32px));padding:0;border:1px solid var(--line);border-radius:22px;color:#29213a;background:#fff;box-shadow:0 24px 70px rgba(53,48,103,.28)}.confirm-dialog::backdrop{background:rgba(40,36,63,.38);backdrop-filter:blur(3px)}.confirm-body{padding:28px}.confirm-icon{width:42px;height:42px;display:grid;place-items:center;margin-bottom:18px;border-radius:14px;background:var(--danger-soft);color:var(--danger);font-size:21px;font-weight:800}.confirm-body h2{margin:0 0 9px}.confirm-body p{margin:0;color:var(--muted);font-size:14px;line-height:1.55}.confirm-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:24px}.confirm-actions button{min-width:100px}
...
@media(max-width:1100px){header{display:flex;align-items:flex-start;flex-direction:column}.header-actions{flex-wrap:wrap}}
@media(max-width:680px){main{padding:22px 14px 60px}header{padding-right:0}.header-actions{align-items:flex-start;flex-direction:column}.grid{grid-template-columns:1fr;gap:14px}.card{padding:20px;border-radius:18px}.days{grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto}.overview{grid-template-columns:1fr}.tabs{display:flex}.tab{flex:1;min-width:0;padding-inline:10px}.student-tools{align-items:stretch;flex-direction:column}.package-row{grid-template-columns:1fr 1fr}.package-row:before{grid-column:1/-1}.confirm-actions{flex-direction:column-reverse}.confirm-actions button{width:100%}}
.support-footer{display:flex;align-items:center;justify-content:center;gap:12px;padding:24px 20px 8px;flex-wrap:wrap;margin-top:16px}.support-footer .bmc-link{display:inline-flex;align-items:center;gap:7px;min-height:36px;padding:8px 16px;background:#FFDD00;border:1px solid #e8c800;border-radius:999px;color:#000;font-size:13px;font-weight:750;text-decoration:none}.support-footer .bug-link{display:inline-flex;align-items:center;min-height:36px;padding:8px 16px;background:var(--purple-soft);border:1px solid #e2d9f7;border-radius:999px;color:#584396;font-size:13px;font-weight:750;text-decoration:none}
</style></head><body><main><header><h1>Lesson scheduling</h1><div class="header-actions">__ADMIN_LINK__</div></header><nav class="tabs" aria-label="Dashboard sections"><button class="tab active" type="button" data-tab="lessons">Lessons</button><button class="tab" type="button" data-tab="settings">Settings</button></nav>
...
<p class="support-footer"><a class="bmc-link" href="https://www.buymeacoffee.com/okserm" target="_blank" rel="noopener noreferrer">☕ Buy me a coffee</a><a class="bug-link" href="/schedule/feedback">Report a bug</a></p><p class="status" id="status"></p></main><dialog class="confirm-dialog" id="confirm-dialog" aria-labelledby="confirm-title" aria-describedby="confirm-message"><div class="confirm-body"><div class="confirm-icon" aria-hidden="true">!</div><h2 id="confirm-title"></h2><p id="confirm-message"></p><div class="confirm-actions"><button class="secondary" id="confirm-back" type="button">Go back</button><button class="danger" id="confirm-action" type="button"></button></div></div></dialog>
<script>
...
let confirmationResolver=null;function requestConfirmation(title,message,actionLabel){const dialog=$('#confirm-dialog');$('#confirm-title').textContent=title;$('#confirm-message').textContent=message;$('#confirm-action').textContent=actionLabel;dialog.showModal();return new Promise(resolve=>{confirmationResolver=resolve})}function settleConfirmation(accepted){const resolve=confirmationResolver;confirmationResolver=null;if($('#confirm-dialog').open)$('#confirm-dialog').close();if(resolve)resolve(accepted)}$('#confirm-back').onclick=()=>settleConfirmation(false);$('#confirm-action').onclick=()=>settleConfirmation(true);$('#confirm-dialog').addEventListener('cancel',event=>{event.preventDefault();settleConfirmation(false)});
...
function selectTab(name){document.querySelectorAll('.tab').forEach(button=>button.classList.toggle('active',button.dataset.tab===name));$('#settings-panel').hidden=name!=='settings';$('#lessons-panel').hidden=name!=='lessons';history.replaceState({},'',`${location.pathname}${location.search}#${name}`)}
...
document.addEventListener('unhandledrejection',e=>{e.preventDefault();status(e.reason?.message||'Something went wrong.')});load().then(()=>{const params=new URLSearchParams(location.search),result=params.get('calendar'),hash=location.hash.slice(1),known=['settings','lessons'];selectTab(result?'settings':known.includes(hash)?hash:'lessons');if(result==='connected')status('Google Calendar connected and calendars loaded.');else if(result)status('Google connected, but calendars could not be loaded. Please reconnect and grant calendar access.');if(result)history.replaceState({},'',`${location.pathname}#settings`)}).catch(e=>status(e.message));
</script></body></html>"""
...
ACCOUNT_CONTROL_CSS = (
    ".page-account{position:fixed;top:20px;right:24px;z-index:20}"
    ".signed-in{display:flex;align-items:center;gap:9px;padding:6px 7px 6px 11px;"
    "border:1px solid #dedff1;border-radius:13px;background:rgba(255,255,255,.94);"
    "box-shadow:0 8px 24px rgba(75,73,145,.1);backdrop-filter:blur(10px)}"
    ".signed-in span{max-width:220px;color:#716d87;font-size:11px;overflow:hidden;"
