...
function renderMetrics(){$('#completed-count').textContent=state.metrics.completed_lessons;$('#monthly-count').textContent=state.metrics.total_lessons;$('#student-count').textContent=state.students.length;$('#earned-income').textContent=`${money(state.metrics.earned_cents)} earned`;$('#projected-income').textContent=`${money(state.metrics.projected_cents)} projected from completed + scheduled lessons`}
