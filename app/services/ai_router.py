...
class AiRouter:
    ...
    async def classify_light_intent(self, *, text: str) -> AiJsonResult:
        ...
        prompt = (
            ...
            "source, start_time, end_time, kind, category, duration_minutes, amount, transaction_type, "
            "new_title.\n"
            ...
            "remove_item, move_item, add_calendar_event, update_calendar_event, remove_calendar_event, "
            "finance_transaction, expense_query, clarify, unknown.\n"
            "Rules:\n"
            ...
            "- Use add_calendar_event when the user explicitly says calendar/cal/event AND wants to add/create/schedule "
            "a calendar event. Set item/title, date_ref, start_time, end_time.\n"
            "- Use update_calendar_event when the user explicitly says calendar/cal/event AND wants to move/change/reschedule "
            "an existing calendar event. Set item/title to the existing event title, date_ref to the event day, "
            "start_time/end_time to the new time if present, and new_title if renaming.\n"
            "- Use remove_calendar_event when the user explicitly says calendar/cal/event AND wants to delete/cancel/remove "
            "an existing calendar event. Set item/title and date_ref.\n"
            ...
            "- Use going_to_store when the user asks what items they need to buy at/from/in a specific store, "
            "or what they need to buy online (e.g. 'what do I need from Lidl?', 'what to buy at Aldi?', "
            "'what do I need to buy online?'). Set store_name to the store name (e.g. 'Lidl') or 'online'.\n"
            ...
            "- Use finance_transaction when the user states a single income or expense with an amount, "
            "e.g. 'petrol 54', 'expense bills €64.50', 'got paid 400'. Put the cleaned merchant/item "
            "name without command words like expense/income/spent in item. Put the amount in amount. "
            "Set transaction_type to expense or income. Category must be one of: Food, Eat Out, Uber, "
            "Gas, Tolls, Public Transport, Sport, Entertainment, Church, Health, Beauty, "
            "Tech & Devices, House Chemicals, Subscriptions, Taxes, Utilities, Other, Income. "
            "Use Uber/Gas/Tolls/Public Transport for commute details.\n"
    ...
    async def _gemini_bank_json(self, *, image_bytes: bytes, mime_type: str) -> AiJsonResult:
        ...
        prompt = (
            ...
            "Uber, Gas, Tolls, Public Transport, Sport, Entertainment, Church, Health, Beauty, "
            "Tech & Devices, House Chemicals, Subscriptions, Taxes, Utilities, Income, Other. "
            "Use Uber/Gas/Tolls/Public Transport instead of a generic Commute category. "
            "PayPal ride/transport bills should be Uber."
        )
        url = (
            "https://generativelanguage.googleapis.com/v1beta/models/"
            f"{self.settings.gemini_model}:generateContent"
